...
namespace RealTimeDataWebUI.Controllers
{
    public class CategoryController : Controller
    {
		...
		public async Task<IActionResult> CreateCategory (Dtos.CategoryDtos.CreateCategoryDto createCategoryDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7021/api/Category", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7021/api/Category/{id}");
            if (responseMessage.IsSuccessStatusCode) 
            {
                return RedirectToAction("Index");
            }
